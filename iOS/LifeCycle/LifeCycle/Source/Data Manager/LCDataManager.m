...
- (void)updateBikesWithCompletion:(void (^)())completion {
    [self sendRequest:@"list_bikes" withData:nil andCompletion:^(NSError *error, NSArray *response) {
        ...
        [response enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            LCUser *user = [[LCUser alloc] initWithUserName:bikeJson[@"name"] profileImageURL:bikeJson[@"photourl"] rating:bikeJson[@"rating"]];
            BOOL rented = ![bikeJson[@"available"] boolValue];
            NSString *code = bikeJson[@"code"];
            NSArray *pictures = bikeJson[@"pictures"];
            LCBike *bike = [[LCBike alloc] initWithID:bikeID location:location owner:user rented:rented code:code pictures:pictures];
